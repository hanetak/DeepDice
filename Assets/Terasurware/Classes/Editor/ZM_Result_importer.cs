...
public class ZM_Result_importer : AssetPostprocessor
{
    private static readonly string filePath = "Assets/Exel/ZM_Result.xls";
    private static readonly string[] sheetNames = { "Result", };
        ...
        foreach (string asset in importedAssets)
        {
            ...
            using (FileStream stream = File.Open (filePath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
            {
                ...
                foreach (string sheetName in sheetNames)
                {
                    ...
                    var data = (Result)AssetDatabase.LoadAssetAtPath(exportPath, typeof(Result));
                    if (data == null)
                    {
                        data = ScriptableObject.CreateInstance<Result>();
